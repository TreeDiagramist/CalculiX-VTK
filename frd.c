...
void frd(double *co,ITG *nk,ITG *kon,ITG *ipkon,char *lakon,ITG *ne0,
  ...
  if((*kode==1)&&((*nmethod!=5)||(*mode!=0))){
    ...
    fprintf(f1,"# vtk DataFile Version 3.0\n");
    fprintf(f1,"\n");
    fprintf(f1,"ASCII\n");
    fprintf(f1,"DATASET UNSTRUCTURED_GRID\n");
    fprintf(f1,"\n");
    //fprintf(f1,"%5sUVERSION           Version 2.17                             \n",p1);
    //fprintf(f1,"%5sUCOMPILETIME       2020年  8月 15日 土曜日 20:26:10 JST                    \n",p1);
    //fprintf(f1,"%5sUDIR                                                               \n",p1);
    //fprintf(f1,"%5sUDBN                                                               \n",p1);
	...
	if(((strcmp1(&lakon[8*i+6]," ")==0)||
	  ...
	  if(strcmp1(output,"asc")==0){
	    ...
	    for(j=0;j<20;j++)fprintf(f1,"%d ",kon[indexe+j]-1);
	    fprintf(f1,"\n");
  ...
  if((*nmethod!=5)||(*mode==-1)){
	...
	fprintf(f1,"POINT_DATA %d\n",nout);
	fprintf(f1,"FIELD attributes 3\n");
	fprintf(f1,"DISPLACEMENT 3 %d double\n",nout);
	//fprintf(f1," -5  D3          1    2    3    0\n");
	//fprintf(f1," -5  ALL         1    2    0    0    1ALL\n");
      
	vtkdisplacement(v,&iset,ntrans,filab,&nkcoords,inum,inotr,
  ...
  if((*nmethod!=5)||(*mode==-1)){
    if((strcmp1(&filab[174],"S   ")==0)&&(*ithermal!=2)){
      ...
      vtkstress(stn,stn,&iset,&nkcoords,inum,istartset,iendset,
		ialset,ngraph,&ncomptensor,ifieldtensor,icomptensor,
		nfieldtensor,&iselect,f1,output);

      fprintf(f1,"MisesStress 1 %d double\n",nout);

      vtkmisesstress(stn,stn,&iset,&nkcoords,inum,istartset,iendset,
		ialset,ngraph,&ncomptensor,ifieldtensor,icomptensor,
		nfieldtensor,&iselect,f1,output);      
