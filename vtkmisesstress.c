...
void vtkmisesstress(double *field1,double *field2,ITG *iset,ITG *nkcoords,ITG *inum,
     ITG *istartset,ITG *iendset,ITG *ialset,ITG *ngraph,ITG *ncomp,
     ITG *ifield,ITG *icomp,ITG *nfield,ITG *iselect,FILE *f1,
     char *output){
  ...
  if(*iset==0){
	...
	for(n=1;n<=(ITG)((*ncomp+5)/6);n++){
	  if(n==1){
	    ...
	    for(j=0;j<min(1,*ncomp);j++){
	      if(ifield[j]==1){
		if(strcmp1(output,"asc")==0){
			Mises=sqrt(0.5*(pow((float)field1[i*6]-field1[i*6+1],2.0)+pow((float)field1[i*6+1]-field1[i*6+2],2.0)+pow((float)field1[i*6+2]-field1[i*6],2.0))+3.0*(pow((float)field1[i*6+3],2.0)+pow((float)field1[i*6+4],2.0)+pow((float)field1[i*6+5],2.0)));
		    fprintf(f1,"%12.5E",Mises);
